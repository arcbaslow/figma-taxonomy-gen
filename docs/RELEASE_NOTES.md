...
- Constrained the MCP extra to SDK 1.x so FastMCP remains importable on a fresh installation.
- Changed the MCP construction test to fail for an installed incompatible SDK instead of silently skipping it.
- Aligned pyproject.toml, __version__ and the local uv.lock package version.
- Corrected the MCP tool list and documented fixture-based extraction, drift checks and immediate Amplitude push behavior.
